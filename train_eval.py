...
#该文件用于对模型进行训练和测试
# 权重初始化，默认xavier
def init_network(model, method='xavier', exclude='embedding', seed=123):#初始化参数
    for name, w in model.named_parameters():#获得所有参数名和权重
        if exclude not in name:
            if len(w.size()) < 2:#如果是一维的权重，就跳过
                continue
            if 'weight' in name:#如果是权重参数，就根据分布类型初始化
                if method == 'xavier':
                    ...
                    nn.init.normal_(w)
            elif 'bias' in name:#如果是偏置参数，就初始化为0
